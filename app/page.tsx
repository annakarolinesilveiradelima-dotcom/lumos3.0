...
function sourceNames(items: IntelligenceItem[]) {
  return [...new Set(items.map((item) => item.source).filter(Boolean))].slice(
    0,
    4
  );
}
  ...
  const groups = [
    ...
    makeInsight(
      ...
      cleanItems.filter(
        (item) =>
          ...
          hasTag(item, ["comparison", "filmes", "risk", "crítica", "x"])
      )
    ),
    ...
    makeInsight(
      ...
      cleanItems.filter(
        (item) =>
          ...
          hasTag(item, ["creators", "youtube", "education", "explicadores"])
      )
    ),
    ...
    makeInsight(
      ...
      cleanItems.filter(
        (item) =>
          item.region === "BR" ||
          hasTag(item, ["brasil", "press", "editorial", "imprensa", "guias"])
      )
    )
...
export default function Page() {
  const [snapshot, setSnapshot] =
    useState<IntelligenceSnapshot>(initialSnapshot);

  const [selectedWeek, setSelectedWeek] = useState(
    initialSnapshot.currentWeekId
  );
  ...
  const [lastUpdateMessage, setLastUpdateMessage] = useState(
    "Aguardando atualização do feed."
  );
  ...
  const currentWeek = useMemo(() => {
    return (
      snapshot.weeks.find((week) => week.weekId === selectedWeek) ||
      snapshot.weeks[snapshot.weeks.length - 1]
    );
  ...
  const scopedItems = useMemo(() => {
    ...
    const cleanItems = snapshot.items
      .filter((item) => !isSearchBackfill(item))
      .filter((item) => isHarryPotterRelevant(item));
    ...
    const exactWeekItems = cleanItems.filter(
      (item) => item.weekId === selectedWeek
    );

    if (exactWeekItems.length > 0) {
      return exactWeekItems;
    }
  ...
  const weeklyFallbackActive =
    period === "week" && exactWeekItemCount === 0 && scopedItems.length > 0;

  const narrativeInsights = useMemo(
    () => buildNarrativeInsights(scopedItems),
    [scopedItems]
  );

  const weeklyNarrativeSummary = useMemo(() => {
    const positive = scopedItems.filter(
      (item) => item.sentiment === "positive"
    ).length;

    const neutral = scopedItems.filter(
      (item) => item.sentiment === "neutral"
    ).length;

    const negative = scopedItems.filter(
    ...
    const summary = scopedItems.length
      ? `Nesta janela, a IA analisou ${scopedItems.length} fonte(s)/sinal(is) sobre Harry Potter. A leitura principal é: ${
          topInsight?.summary ||
          topEvidence?.summary ||
          currentWeek?.keyNarrative
        }`
  ...
  const filteredCoverage = useMemo(() => {
    return scopedItems.filter((item) => {
      ...
      if (["positive", "neutral", "negative"].includes(coverageFilter)) {
        return item.sentiment === coverageFilter;
      }
    ...
    <div className="app-shell">
      <aside className="side">
        ...
        <nav>
          {navItems.map(({ key, label, icon: Icon }) => (
            ...
            >
              ...
              {key === "risks" && (
                <span className="badge">{riskItems.length}</span>
              )}
            </button>
          ))}
        </nav>
        ...
        <div className="sidefoot">
          ...
          <br />
          Atualizado:{" "}
          <b>{new Date(snapshot.generatedAt).toLocaleString("pt-BR")}</b>
          <br />
      ...
      <main className="main">
        <header className="top">
          ...
          <button className="btn" onClick={updateIntelligence} disabled={loading}>
            <RefreshCw size={15} />
            {loading ? "Atualizando" : "Recarregar feed"}
          </button>

          <button className="btn gold" onClick={() => exportFile("pdf")}>
            Relatório
          </button>
        </header>
        ...
        <div className="content">
          {activeView === "overview" && (
            <section className="view active">
              <div className="head">
                ...
                <p>
                  Consolidação semanal com matérias clicáveis, fontes,
                  sentimento, risco e oportunidade.
                </p>
              </div>
              ...
              <div className="executive-readout">
                ...
                <h3>
                  {weeklyNarrativeSummary.topInsight?.title ||
                    "Monitoramento ativo de Harry Potter"}
                </h3>

                <p>{weeklyNarrativeSummary.summary}</p>

                {weeklyFallbackActive && (
                  <p className="fallback-note">
                    Sem novas fontes exatamente em {selectedWeek}; exibindo os
                    sinais mais recentes anteriores para manter contexto semanal.
                  </p>
                )}

                <div className="readout-sources">
                  {weeklyNarrativeSummary.topSources
                    .slice(0, 5)
                    .map((source) => (
              ...
              <div className="kpis">
                <div className="kpi">
                  <small>Matérias / Sinais</small>
                  <strong>{scopedItems.length}</strong>
                  <span className="smallnote">janela selecionada</span>
                </div>

                <div className="kpi">
                  <small>Sentimento positivo</small>
                  <strong>
                    {pct(
                      scopedItems.filter((item) => item.sentiment === "positive")
                        .length,
                      scopedItems.length
                    )}
                    %
                  </strong>
                  <span className="delta up">+ memória semanal</span>
                </div>

                <div className="kpi">
                  <small>Risco médio</small>
                  <strong>
                    {currentWeek?.riskScore || 0}
                    <span>/100</span>
                  </strong>
                  <span className="smallnote">radar de fandom</span>
                </div>

                <div className="kpi">
                  <small>Buzz score</small>
                  <strong>
                    {currentWeek?.buzzScore || 0}
                    <span>/100</span>
                  </strong>
                  <span className="smallnote">desde o teaser</span>
                </div>
              </div>
                        ...
                        .map((insight) => (
                          ...
                          >
                            <div>
                              <b>{insight.title}</b>
                              <small>
                                {insight.evidence.length} evidência(s) · fontes:{" "}
                                {insight.sourceNames.join(", ")}
                              </small>
                            </div>
          ...
          {activeView === "narratives" && (
            <section className="view active">
              <div className="head">
                ...
                <p>
                  Síntese executiva da semana selecionada, com fontes usadas
                  pela IA e evidências clicáveis.
                </p>
              </div>
              ...
              <div className="narrative-summary">
                <div>
                  ...
                  {weeklyFallbackActive && (
                    <p className="fallback-note">
                      Sem novas fontes exatamente em {selectedWeek}; exibindo os
                      sinais mais recentes anteriores para manter contexto
                      semanal.
                    </p>
                  )}
                </div>
                ...
                <div className="narrative-scorecard">
                  <div>
                    ...
                    <b>{weeklyNarrativeSummary.positive}</b>
                  </div>

                  <div>
                    <small>Neutro</small>
                    <b>{weeklyNarrativeSummary.neutral}</b>
                  </div>

                  <div>
                    <small>Crítico</small>
                    <b>{weeklyNarrativeSummary.negative}</b>
                  </div>
                </div>
              </div>
              ...
              <div className="grid2b">
                        ...
                        .map((insight) => (
                          <div className="insight-row" key={insight.id}>
                            <div className="meta">
                              ...
                              <span className="pill nos">
                                Opp {insight.opportunityScore}
                              </span>
                            ...
                            <small>
                              Por que importa: {insight.whyItMatters}
                            </small>
                          </div>
                        ))
                    ) : (
                      <div className="empty">
                        Sem leitura consolidada nesta semana.
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="panel">
                  ...
                  <div className="list">
                    {weeklyNarrativeSummary.topSources.length ? (
                      weeklyNarrativeSummary.topSources.map((source) => (
              ...
              <div className="section-subhead">
                ...
                <h3>
                  Top {Math.min(scopedItems.length, EVIDENCE_DISPLAY_LIMIT)}{" "}
                  itens que sustentam o resumo
                </h3>
                <p>
                  Filtramos o excesso e mostramos só os sinais mais relevantes
                  da janela. Total disponível: {scopedItems.length}.
                </p>
              </div>
              ...
              <div className="cards">
                {displayedEvidence.length ? (
                  displayedEvidence.map((item) => (
                    {cleanUrl(item.url)}                      target="_blank"
                      rel="noreferrer"
                    >
                      <h4>{item.title}</h4>
                      ...
                      <div className="meta">
                        <span
                          className={`pill ${sentimentClass(item.sentiment)}`}
                        >
                      ...
                      <small>
                        Fonte usada pela IA: {item.source} · {actionLabel(item)}{" "}
                  ...
                  <div className="empty" style={{ gridColumn: "1 / -1" }}>
                    Sem evidências suficientes para esta semana.
                  </div>
                )}
              </div>
            </section>
          ...
          {activeView === "coverage" && (
            <section className="view active">
              <div className="head">
                <div>
                  <span className="eyebrow">PR Listening</span>
                  <h2>Cobertura & fontes</h2>
                </div>

                <p>
                  Matérias, posts e fontes reais. Cada linha abre o link original.
                </p>
              </div>
              ...
              <div className="filters">
                {(
                  ...
                  <button
                    key={filter}
                    className={`chip ${coverageFilter === filter ? "on" : ""}`}
              ...
              <div className="coverage-toolbar">
                <span>
                  Mostrando top {displayedCoverage.length} de{" "}
                  {filteredCoverage.length} fonte(s) filtradas por relevância.
                </span>
              </div>

              <div className="coverage list">
                {displayedCoverage.length ? (
                  displayedCoverage.map((item) => (
                    {cleanUrl(item.url)}                      target="_blank"
                      rel="noreferrer"
                    >
                      <span className="time">
                  ...
                  <div className="empty">
                    Sem fontes de Harry Potter nesta semana. O Lumos agora
                    filtra notícias fora do tema e pode mostrar contexto anterior
                    quando a semana estiver vazia.
                  </div>
                )}
              </div>
            </section>
          ...
          {activeView === "sentiment" && (
            <section className="view active">
              <div className="head">
                <div>
                  <span className="eyebrow">Sentimento</span>
                  <h2>Como a conversa está reagindo</h2>
                </div>

                <p>
                  Percentual positivo/neutro/crítico baseado nos sinais
                  disponíveis.
                </p>
              </div>
              ...
              <div className="kpis three">
                {sentimentData.map((entry) => (
                  <div className="kpi" key={entry.name}>
                    <small>{entry.name}</small>
                    <strong>{pct(entry.value, scopedItems.length)}%</strong>
                    <span className="smallnote">{entry.value} sinais</span>
                  </div>
                ))}
              </div>

              <div className="panel">
                <h3>Sentimento por semana</h3>

                <div className="canvasbox">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={snapshot.weeks.slice(-12)}>
                      ...
                      />
                      <Bar dataKey="positive" stackId="a" fill="#63c08c" />
                      <Bar dataKey="neutral" stackId="a" fill="#8a90a6" />
                      <Bar dataKey="negative" stackId="a" fill="#da6e6e" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
                ...
                <tbody>
                  {snapshot.creators.map((creator) => (
                    <tr key={creator.name}>
                      ...
                      <td>{creator.fit}</td>
                      <td>{creator.score}/100</td>
                      <td>{creator.risks.join(" ")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {activeView === "risks" && (
            <section className="view active">
              <div className="head">
                <div>
                  <span className="eyebrow">Radar de riscos</span>
                  <h2>Pontos de atenção</h2>
                </div>

                <p>Riscos derivados das conversas e matérias monitoradas.</p>
              </div>
            ...
            <section className="view active">
              <div className="head">
                <div>
                  ...
                  <h2>Onde surfar a conversa</h2>
                </div>

                <p>Itens com maior potencial de PR, social, creators e CRM.</p>
              </div>

              <div className="hero">
                <span className="eyebrow">Insight aplicável</span>
                ...
                <p>
                  {weeklyNarrativeSummary.topInsight?.whyItMatters ||
                    currentWeek?.whatChanged}
                </p>

                <div className="facts">
                  <div>
                    <small>Semana</small>
                    <b>{currentWeek?.weekId}</b>
                  </div>

                  <div>
                    <small>Opportunity</small>
                    <b>{currentWeek?.opportunityScore}/100</b>
                  </div>

                  <div>
                    <small>Buzz</small>
                    <b>{currentWeek?.buzzScore}/100</b>
                  </div>
                </div>
              </div>
          ...
          {activeView === "assistant" && (
            <section className="view active">
              <div className="head">
                ...
                <p>Resposta contextual baseada na semana selecionada.</p>
              </div>

              <div className="panel assistant-box">
                <Search size={20} />

                <p>
                  <b>Leitura Lumos:</b> {weeklyNarrativeSummary.summary} Próximo
                  passo: abrir as evidências da aba Narrativas e priorizar as
                  fontes com maior recorrência para briefing de PR/social.
                </p>
              </div>
            </section>
          )}
